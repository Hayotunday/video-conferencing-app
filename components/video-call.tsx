...
import { type Call, StreamCall, SpeakerLayout, useCallStateHooks, useCall } from "@stream-io/video-react-sdk"
...
function VideoCallContent({ onLeave }: { onLeave?: () => void }) {
  ...
  const meetingTitle = call?.state?.custom?.title || `Meeting Room`
  ...
  }, [call, onLeave, router, transcript])
  ...
  const participantEmails = participants.map((p) => p.custom?.email || "").filter(Boolean)

  const participantNames = participants.map((p) => p.name || p.userId).filter(Boolean)
    ...
    <div className="h-screen flex flex-col bg-gray-900">
      ...
      <header className="flex items-center justify-between p-4 bg-gray-800 text-white border-b border-gray-700">
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm text-gray-300">Meeting ID: {roomId}</span>
        </div>
        ...
        <div className="flex items-center gap-4 text-white">
          ...
          <span className="text-xs text-gray-300 capitalize">{callingState}</span>
        ...
        {showParticipants && (
          <div className="w-80 bg-gray-800 text-white p-4 border-l border-gray-700">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-white">Participants ({participantCount})</h3>
              <MeetingShare roomId={roomId} title={meetingTitle}>
            ...
            <div className="space-y-2">
              {participants.map((participant) => (
                <div key={participant.sessionId} className="flex items-center gap-3 p-2 rounded-lg bg-gray-700">
        ...
        <div className="flex items-center justify-center gap-4">
          ...
          <Button
            ...
            className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${
              !isAudioMuted ? "bg-gray-600 hover:bg-gray-500 text-white" : "bg-red-600 hover:bg-red-500 text-white"
            }`}
            title={!isAudioMuted ? "Mute microphone" : "Unmute microphone"}
          >
            {!isAudioMuted ? <Mic className="h-5 w-5" /> : <MicOff className="h-5 w-5" />}
          </Button>
          ...
          <Button
            onClick={toggleVideo}
            className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${
              !isVideoMuted ? "bg-gray-600 hover:bg-gray-500 text-white" : "bg-red-600 hover:bg-red-500 text-white"
            }`}
            title={!isVideoMuted ? "Turn off camera" : "Turn on camera"}
          >
            {!isVideoMuted ? <Video className="h-5 w-5" /> : <VideoOff className="h-5 w-5" />}
          </Button>
